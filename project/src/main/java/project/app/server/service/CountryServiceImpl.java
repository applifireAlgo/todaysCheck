...
@SourceCodeAuthorClass(createdBy = "puja.singh@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Service for Country Master table Entity", complexity = Complexity.LOW)
...
public class CountryServiceImpl extends CountryService {
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            project.app.shared.location.Country lstcountry = countryrepo.findById((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lstcountry);
        } catch (org.springframework.transaction.TransactionException e) {
